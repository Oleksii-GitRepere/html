...
setInterval(()=>
{
    ...
    }else if(window.screen.availWidth > 950 )
    {
        ...
        carousel_wrapper.innerHTML =  carousel_slides.values().next + carousel_slides.values().next + carousel_slides.values().next;//(num+1 > carousel_slides.length-1) ? (carousel_slides[num] + carousel_slides[num] + carousel_slides[0]) : (carousel_slides[num] + carousel_slides[num+1] + carousel_slides[num+2]); //+carousel_slides[1];
    }
} 
,300);